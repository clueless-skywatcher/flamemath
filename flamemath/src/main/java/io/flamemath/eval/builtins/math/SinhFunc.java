...
public class SinhFunc implements FlameFunction {
    ...
    public Expr apply(List<Expr> args, FlameValuator evaluator) throws Exception {
        ...
        if (arg instanceof IntegerAtom i) {
            return toNumericAtom(Math.sinh(i.value().toDouble()));
        }
        if (arg instanceof RealAtom r) {
            return toNumericAtom(Math.sinh(r.value()));
        }
    ...
    public Expr numerify(List<Expr> args) throws Exception {
        return toNumericAtom(Math.sinh(FlameUtils.numericValue(args.get(0))));
    }
}
